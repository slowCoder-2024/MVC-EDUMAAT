...
"footerCallback": function ( row, data, start, end, display ) {
 ...
 /* $( api.column( 2 ).footer() ).html(
  ...
  $("#academicfinepagetotal").html(pageTotalAmount);
	...
	$.ajax({
		   url:ctx+'/report/currentacademicyearacademicfeesreportlist',
		   type:'GET',
	...
	$("#getLedgerList").click(function(event) 
	{
		...
		if($("#dateDetailForm").valid())
		{
			...
			$.ajax({
		   url:ctx+'/report/academicfeesreportlist',
	...
	$('#ledgerAccountHeadList').on( 'click', 'tr td a#edit', function () {
		...
		$('.loader').show();
			$.ajax({
			   url:ctx+'/institution/ledgerAccount/ledgerAccountNameAndAcademicYear',
