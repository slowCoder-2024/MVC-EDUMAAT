$(document).ready(function() {
	 ...
	 $(document).on('click',".edittdd",function (event){   
	   ...
	   $('#substituteTimeTableGeneratorList').on( 'click', 'tr td a#delete', function () {
         	 var deleteSubstituteTimeTableGeneratorId = $(this).attr('data-id');
         	 $('#confirm_delete_substituteTimeTableGenerator').on('show.bs.modal', function (e) {
         		 $("#confirmDeleteSubstituteTimeTableGenerator").click(function(event) {
         			 $("#deleteSubstituteTimeTableGeneratorId").val(deleteSubstituteTimeTableGeneratorId);
         			 $('.loader').show();
         			 $("#deleteSubstituteTimeTableGeneratorForm").submit();  
         		});
         		});
      	...
      	$("#edittimetableview").append(' <h4 class="title1">Class Name : <label for="defaultClassName"  id="defaultClassName"></label> &nbsp;&nbsp;&nbsp;&&&nbsp;&nbsp;&nbsp; Section Name : <label for="defaultSectionName"  id="defaultSectionName"></label></h4><br>');  
       		...
       		$.ajax({
      					...
      					var rowspancount=0;
       					 ...
       					 */ //alert(timeTableTemplateHours);
							...
							$("#defaultSectionName").html(response.classSection.sectionClass.sectionName);
       	        		  $.ajax({
             				   url:ctx+'/classSection/classSectionModule/substitute',
    				   ...
    				   error: function(){
    					   $('.loader').hide();
    					   alert('ERROR OCCURED');
    					   window.location.href=ctx+"/timetable/generator/substitute";
    			       }
       ...
       $('#savetimetable').click(function(event){
	 	...
	 	$('#sectionId').change(function(event) {
	 		...
	 		if(sectionId!=null){
	 			...
	 			$.get(ctx+"/classSection/classSectionModule/substitute",data,function(response){
	 				if($.trim(response)&&response!=null){
	 					var select = $('#moduleId');
