				        	  ...
				        	  datatable.clear().draw();
								$.each(data, function (i, item) {
									...
									$.each(item.studentInvoiceDetails, function (j, studentInvoiceDetail) {
										...
										if(studentInvoiceDetail.studentPartialPaymentReceiptDetail && studentInvoiceDetail.studentInvoiceElementPaymentStatus!=0)
										{
											paidInvoiceAmount+=studentInvoiceDetail.studentPartialPaymentReceiptDetail.paidReceiptAmount + studentInvoiceDetail.studentPartialPaymentReceiptDetail.discountAmount;
										}
										else
										{
				...
				/* $('#finalPaymentDetailTable tbody').on( 'click', 'tr', function () {
		         ...
		         /*  $(".form-horizontal").trigger('reset'); */
		           ...
		           $(".loader").show();
						if(invoiceForPayment.length>0){
							$.get(ctx+'/invoice/invoicedetailsforpayment',{invoiceForPayment:invoiceForPayment},function(response){
								...
								/*	if(i==0){
										...
										$.get(ctx+'/invoice/studentInvoice/itemdetailsandstatus',{studentInvoiceId:finalStudentInvoices.studentInvoiceId},function(invoicedetails){
								    	 ...
								    	 $.each(invoicedetails, function (k,invoicedetail) {
											if(invoicedetail.studentInvoiceElementPaymentStatus==1){
													if(checkBoxValue=="Partial")
													{
														...
														if(invoicedetail.studentPartialPaymentReceiptDetail && invoicedetail.studentInvoiceElementPaymentStatus!=0)
														{
						...
						{
							if($("#payment"+partialPaymentId).val()>0)
							{
						...
						$(document).on('change',".payment",function (event){   
							...
							{
								if($("#discountFlat"+flatPaymentId).val()>0)
								{
						...
						$('#paymentDetailsPanelForm').attr('action', ctx+'/receipt/generate/testingpayment');
						$("#productinfo").val('Nothing');
						$("#surl").val('https://localhost:8080'+ctx+'/receipt/generate');
						$("#furl").val('https://localhost:8080'+ctx+'/receipt/generate');
						$("#pg").val('NB');
			        	...
			        	/* $(".form-horizontal").trigger('reset'); */
			        	 datatable.clear().draw();
							...
							$.each(data, function (i, item) {
								...
								if(item.studentPartialPaymentReceiptDetail && item.studentInvoiceElementPaymentStatus!=0)
								{
									partialPaymentReceiptAmount=partialPaymentReceiptAmount+item.studentPartialPaymentReceiptDetail.paidReceiptAmount+item.studentPartialPaymentReceiptDetail.discountAmount;
								}
								else
								{
